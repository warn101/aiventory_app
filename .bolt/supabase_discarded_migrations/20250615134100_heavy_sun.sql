/*
  ...
  1. New Tables
    ...
    - `tools` - AI tools with ratings, reviews, and metadata
  ...
  2. Security
    ...
    - Add policies for public read access on categories and tools
    - Add policies for authenticated user access on profiles, bookmarks, and reviews
...
-- Add foreign key constraint for tools.category if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Add foreign key constraint for profiles.id if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- Create full-text search function if it doesn't exist
...
CREATE TRIGGER on_tool_category_change
  ...
  FOR EACH ROW EXECUTE FUNCTION update_tool_category_change();